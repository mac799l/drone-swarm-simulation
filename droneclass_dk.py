...
class drone():
    ...
    def __init__(self, vehicle, state_vector, avoidance):
        ...
        if self.avoidance:
            ...
            for x in range(0,duration):
                ...
                for state in self.state_vector:
                    distances.append( utils.gpsDistance(curr_gps, state.get("gps")) )
                for i, distance in enumerate(distances):
                    if distance is None:
                        print(f"Error computing drone distance. Invalid GPS: '{distance}'.")
                        continue
                    print(f"Distance to drone #{i}: {distance}")
                    if distance < MIN_DISTANCE_MI:
                        # TODO: CORRECT TRAJECTORY
                        print("Too close to another drone!")
                time.sleep(1)
        else:
            # send command to vehicle on a 1 Hz cycle
            for x in range(0,duration):
    ...
    def go_to(self, target):
        ...
        while not at_target:
            ...
            for i in range(2):
                ...
                if (curr_position < curr_target * MIN_TOLERANCE 
                    or curr_position > curr_target * MAX_TOLERANCE):
                    at_target = False
            time.sleep(1)
